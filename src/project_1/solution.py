...
def to_vector(q):
    return [v for vs in map(raw_embs.get, simple_preproc(q)) for v in vs if vs is not None]
...
def update_index():
    ...
    for k, q in state['documents'].items():
        vectors.append(to_vector(q))
        state['mapping'].append(k)
        i += 1

    state['max_dim'] = max(map(len, vectors))
    padded_vectors = [v + [0.0] * (state['max_dim'] - len(v)) for v in vectors]
    x = int(2 * math.sqrt(i))
...
def handle_query():
    ...
    for q in request.json['queries']:
        ...
        if not is_en:
            result['suggestions'].append(None)
            continue

        vector = to_vector(q)
        _, ids = state['index'].search(vector[:state['max_dim']] + [0.0] * (state['max_dim'] - len(vector)), 10)
        suggestions = []

        for id in ids:
            k = state['mapping'][id]
            suggestions.append((k, state['documents'][k]))

        result['suggestions'].append(suggestions)
